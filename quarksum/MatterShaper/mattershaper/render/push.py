...
def push_render(objects, camera, light, density=200, bg_color=None):
    ...
    for obj in objects:
        ...
        for node in nodes:
            ...
            # Depth test with splatting: each node covers a small footprint
            # based on its distance from camera. This fills gaps between
            # sparse surface nodes without requiring infinite density.
            z = (node.position - camera.pos).dot(camera.forward)

            # Splat radius: ~1 node-spacing projected into screen space
            ...
            splat_r = min(splat_r, 4)  # cap to avoid blobs
