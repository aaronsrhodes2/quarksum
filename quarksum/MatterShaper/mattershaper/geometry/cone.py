...
Ray-cone intersection: transform to local frame, solve the quadratic
...
class Cone:
    ...
    def intersect(self, ray, t_min=0.001, t_max=float('inf')):
        ...
        best_hit = self._intersect_cap(
            ray, oc_local, dir_local, 0, self.base_radius,
        ...
        if self.top_radius > 0:
            best_hit = self._intersect_cap(
                ray, oc_local, dir_local, self.height, self.top_radius,
                Vec3(0, 1, 0), t_min, t_max, best_hit
            )

        return best_hit

    def _intersect_cap(self, ray, oc_local, dir_local, y_plane, cap_radius,
