...
Ray-ellipsoid intersection: transform ray into the ellipsoid's
...
import math
from .primitives import Vec3, Ray, Hit
...
def _mat_mul(A, B):
    """Multiply two 3x3 matrices (each a tuple of 3 row-Vec3s).
    Returns tuple of 3 row-Vec3s."""
    # B columns
    b0 = Vec3(B[0].x, B[1].x, B[2].x)
    b1 = Vec3(B[0].y, B[1].y, B[2].y)
    b2 = Vec3(B[0].z, B[1].z, B[2].z)
    return (
        Vec3(A[0].dot(b0), A[0].dot(b1), A[0].dot(b2)),  # Actually no...
    )
...
class Ellipsoid:
    ...
    def intersect(self, ray, t_min=0.001, t_max=float('inf')):
        """Ray-ellipsoid intersection with rotation support.

        Strategy:
        1. Translate ray to ellipsoid-centered coords
        2. Rotate ray into ellipsoid's local frame (R^T)
        ...
        dir_local = _apply(self.rotation_T, ray.direction)
